...
public static void verifyUrl(String expectUrl)
{
	...
	soft=new SoftAssert();
	soft.assertEquals(currentUrl, expectUrl);
	soft.assertAll();
}

public static void verifyText(WebElement element) {
	//print get  text
...
//public void openUrl() {
//	driver.get(PropertyReader.getConfigProperty("url"));
//	String expect_url="https://web.mahakaleshwarujjain.in/";
//	  log.info("url opend");
//}
...
public  static void scrollBardown(int position) throws InterruptedException 
{
	Thread.sleep(3000);
...
public static void scrollByWebElement(WebElement element) throws InterruptedException {
	Thread.sleep(3000);
...
// send keys 
     public static void sendKeys(String elementName,WebElement element, String testData,int timeout) 
 	{
 		...
 		element2.sendKeys(testData);
 		 //Extent.getTest().log(Status.PASS,"Enter Value in -- "+element.getText()+"Text Field"+ "Test Data is : " +text);
 		 Extent.getTest().log(Status.PASS,"Enter Value in-- " +elementName+"  "+element2.getText()+ "Test Data is - " + testData +  "Step Passed");
 		}
 		catch (Exception e) {
 		e.printStackTrace();
        Extent.getTest().log(Status.FAIL,"Enter Value in--"+elementName+" "+element2.getText()+ "Test Data is- " + testData +  "Step Failed");
 		}
 		}
    
     
     // clickon
     public static void clickOn( String elementName,WebElement element,int timeout) 
 	{
 		...
 		try
 		{
 		 ...
 		 element1.click();
        // Extent.getTest().log(Status.INFO,"Click On="+element.getText());
       //  Extent.getTest().log(Status.PASS,"Click On -->"Element Name---" +elementName  +element1.getText()+ "->Step Passed");
 		  Extent.getTest().log(Status.PASS, "Click On--"+elementName+" "+element1.getText()+"-->Step Passed");
 		}
 		catch (Exception e) 
 		{
 		e.printStackTrace();
